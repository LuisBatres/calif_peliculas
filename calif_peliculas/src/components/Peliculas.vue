            ...
            <v-card>
                <v-card-title primary-title>
                    <div>
                        ...
                        <span class = "grey--text">{{ pelicula.anhopub }} &middot; {{ pelicula.genero }}</span>
                    </div>
                </v-card-title>
                <!--<h6 class="card-tittle"
                v-if="current_user">Calificar esta pelicula</h6>-->
                <h6 class="card-title" @click="calificar">Calificar esta pelicula</h6>
                <v-card-text>
                    {{ pelicula.sinopsis }}
                </v-card-text>
            </v-card>
...
import Vue from 'vue';
import StarRating from 'vue-start-rating';
...
// crear componente en contendio
const ComponeneCalif = Vue.extend({
    data(){
        return { calif:0 };
    },
    watch: {
    ...
    template: `
        <div class="rating">
            Cual fue su experiencia viendo esta pelicula?
...
const componente = new ComponeneCalift().$mount(wrapper);
        ...
        async calificar() {
            this.$swal({
                ...
                .catch((error) => {
                    ...
                    this.$swal('Oh no puede ser!', `${mensaje}`,
                    'error');
                });
            });
        },
